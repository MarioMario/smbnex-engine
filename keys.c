...
Key_Message (int key, int ascii)
{
	if (key == K_ENTER || ascii == 10 || ascii == 13)
	{
		if(chat_mode < 0)
			...
			Cmd_ExecuteString(va("%s %s", chat_mode ? "menu_cmd Arblogenchfeglificabilitudinitatibus say_team" : "menu_cmd Arblogenchfeglificabilitudinitatibus say ", chat_buffer), src_command);
